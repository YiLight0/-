...
class performance(object):  
   def __init__(self, name,time,location,placard):  
       self.name = name
       self.time = time 
...
def crawler(city,sort):
    ...
    result_img = bs.find_all('img',class_="l cc-haibao") 
    lenth = len(result_l)
    name = {}   #演出名称
    time = {}   #演出时间
    location = {}   #演出地点
    ...
    for i in range(lenth):
        placard[i] = fun.getimg(result_img[i])
    name,time,location,placard = fun.deleteAD(name,time,location,placard,lenth)
    st.balloons()
    for i in range(lenth):
        col1 , col2 = st.columns([1,2])
        with col1:
            #print(placard[i])
            #r = fun.getimg2(placard[i])
            image = requests.get(placard[i],stream=True).raw
            image = Image.open(image)
            st.image( image, caption=name[i],width=200)
        with col2:
            st.subheader('')
            st.subheader(name[i])
            st.markdown('演出时间 :  ' + time[i])
            st.markdown('演出地点 :  ' + location[i])
            city_ = fun.getCity(location[i])
            lon,lat = fun.getLL(city_)
            df = pd.DataFrame(np.random.randn(50, 2) / [50, 50] + [lat, lon],columns=['lat', 'lon'])
            button_ = st.button('查看地图'+str(i+1))
        if(button_):
            st.map(df)
